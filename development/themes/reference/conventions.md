...
To ensure visual consistency and seamless compatibility with potential future updates, we highly recommend structuring your theme's Tailwind color palette using the following naming conventions in your CSS variables and Tailwind configuration:

- **`billmora-neutral-50` to `billmora-neutral-950`** for neutral shades (used for backgrounds, borders, secondary text, input fields, etc.).
- **`billmora-primary-50` to `billmora-primary-950`** for primary brand/accent colors.

By using these standard scales, implementing dark mode is as simple as overriding the CSS variable values inside a `.dark` selector (or `@media (prefers-color-scheme: dark)`), preventing the need to duplicate your Blade template markup.
