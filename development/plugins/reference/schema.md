...
description: A guide to Billmora auto-rendering UI schema engine for dynamically building settings and forms via PHP arrays.
---
...
Billmora core engine parses this array and renders the beautifully styled HTML components (inputs, selects, toggles) directly into the UI, complete with automatic validation handling.
...
The schema is an associative array where the **key** is the unique identifier (database column/JSON key) for the setting, and the **value** is an array defining the field properties.
...
| `default` | `mixed` | **Optional.** The default value pre-filled if the user/admin has not configured it yet. |
...
By adopting this standardized schema engine across the framework, Billmora ensures that whether you are writing a Gateway plugin, configuring a Server Provisioning, adding a Domain Registrar, or adding dynamic fields to a Store Package checkout, the UI remains perfectly beautiful, perfectly validated, and completely uniform—all without ever touching a single blade file.
